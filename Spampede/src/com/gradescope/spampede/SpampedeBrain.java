...
public class SpampedeBrain extends SpampedeBrainParent {
	 ...
	 *  <p>
	 *  Called by cycle(), test_eatSpam(), and testnoSpamEaten(). 
	 */
	void updateSnake() {
	...
	public void advanceTheSnake(BoardCell nextCell) {
		...
		if (nextCell.isWall() || nextCell.isBody()) {
			...
			this.theData.moveForward(nextCell);
		} else {
			this.theData.moveForward(nextCell);
			this.theData.destroyTail();
		}
	...
	public BoardCell getNextCellFromBFS() {
		...
		BoardCell nextMove = null; 
		
		
		// Search!

		while(!cellsToSearch.isEmpty()) {
			closestSpamCell = cellsToSearch.remove();
			if(closestSpamCell.isSpam()) {
				nextMove = getFirstCellInPath(closestSpamCell);
				break;
			}
			if(!this.theData.getNorthNeighbor(closestSpamCell).inSearchListAlready() && !this.theData.getNorthNeighbor(closestSpamCell).isWall() && !this.theData.getNorthNeighbor(closestSpamCell).isBody()) {
				this.theData.getNorthNeighbor(closestSpamCell).setAddedToSearchList();
				cellsToSearch.add(this.theData.getNorthNeighbor(closestSpamCell));
				this.theData.getNorthNeighbor(closestSpamCell).setParent(closestSpamCell);
			}
			if(!this.theData.getSouthNeighbor(closestSpamCell).inSearchListAlready() && !this.theData.getSouthNeighbor(closestSpamCell).isWall() && !this.theData.getSouthNeighbor(closestSpamCell).isBody()) {
				this.theData.getSouthNeighbor(closestSpamCell).setAddedToSearchList();
				cellsToSearch.add(this.theData.getSouthNeighbor(closestSpamCell));
				this.theData.getSouthNeighbor(closestSpamCell).setParent(closestSpamCell);
			}
			if(!this.theData.getEastNeighbor(closestSpamCell).inSearchListAlready() && !this.theData.getEastNeighbor(closestSpamCell).isWall() && !this.theData.getEastNeighbor(closestSpamCell).isBody()) {
				this.theData.getEastNeighbor(closestSpamCell).setAddedToSearchList();
				cellsToSearch.add(this.theData.getEastNeighbor(closestSpamCell));
				this.theData.getEastNeighbor(closestSpamCell).setParent(closestSpamCell);
			}
			if(!this.theData.getWestNeighbor(closestSpamCell).inSearchListAlready() && !this.theData.getWestNeighbor(closestSpamCell).isWall() && !this.theData.getWestNeighbor(closestSpamCell).isBody()) {
				this.theData.getWestNeighbor(closestSpamCell).setAddedToSearchList();
				cellsToSearch.add(this.theData.getWestNeighbor(closestSpamCell));
				this.theData.getWestNeighbor(closestSpamCell).setParent(closestSpamCell);
			}
	...
	private BoardCell getFirstCellInPath(BoardCell start) {
		if(this.theData.getSnakeHead().getRow() == start.getParent().getRow() && this.theData.getSnakeHead().getColumn() == start.getParent().getColumn()) {
			return start; 
		}
	...
	public void reverseSnake() {
		this.theData.headUnlabeled();
		this.theData.reverseBody();
		this.theData.headRelabeled();
