...
class SpampedeData {
	...
	public void moveForward(BoardCell nextCell) {
		BoardCell head = this.getCell(nextCell.getRow(), nextCell.getColumn());
		BoardCell oldHead = this.getCell(this.getSnakeHead().getRow(), this.getSnakeHead().getColumn());
		this.snakeCells.addLast(head);
		head.becomeHead();
		oldHead.becomeBody();
	...
	public void destroyTail() {
		BoardCell tail = this.getCell(this.getSnakeTail().getRow(), this.getSnakeTail().getColumn());
		tail.becomeOpen();
	...
	public BoardCell getNextCellInDir() {
		SnakeMode dir = this.currentMode; 
		if (dir == SnakeMode.GOING_EAST) {
			return getEastNeighbor(); 
		}else if (dir == SnakeMode.GOING_WEST) {
			return getWestNeighbor(); 
		}else if (dir == SnakeMode.GOING_NORTH) {
	...
	public void reverseBody() {
		reverseBody(snakeCells);
	}
	public void reverseBody(LinkedList<BoardCell> List) {
		if(!List.isEmpty()) {
			BoardCell first = this.getCell(List.getFirst().getRow(), List.getFirst().getColumn());
			List.removeFirst();
			reverseBody(List);
			List.addLast(first);
		}		
	...
	public void headRelabeled() {
		BoardCell newHead = this.getCell(snakeCells.getLast().getRow(), snakeCells.getLast().getColumn());
		newHead.becomeHead();
	}

	// Step 4: calculate the new direction after reversing!
	public void newDirection() {
		BoardCell neck = this.getCell(this.getSnakeNeck().getRow(), this.getSnakeNeck().getColumn());
		BoardCell newHead = this.getCell(snakeCells.getLast().getRow(), snakeCells.getLast().getColumn());
		if (neck.getRow() < newHead.getRow()) {
			setDirectionSouth();
		}else if (neck.getRow() > newHead.getRow()) {
			setDirectionNorth();
		}else if (neck.getColumn() < newHead.getColumn()) {
			setDirectionEast();
		}else {
			setDirectionWest();
		}
